...
)
{
	...
	while (1) {
		...
		if (strcmp(lineHeader, "v") == 0) {
			...
			for (int i = 0; i < 3; i++) {
				vertexKeys[i] = { vertexIndex[i] - 1, uvIndex[i] - 1, normalIndex[i] - 1 };
				if (uniqueVertexMap.find(vertexKeys[i]) == uniqueVertexMap.end()) {
					uniqueVertexMap[vertexKeys[i]] = static_cast<GLuint>(out_vertices.size());
					out_vertices.push_back(temp_vertices[vertexKeys[i].posIndex]);
					out_uvs.push_back(temp_uvs[vertexKeys[i].uvIndex]);
					out_normals.push_back(temp_normals[vertexKeys[i].normalIndex]);
				}
				out_indices.push_back(uniqueVertexMap[vertexKeys[i]]);
			}
		}
	}
